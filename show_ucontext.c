...
main(int argc, char **argv)
{
	 ...
	 * make sure to store it on the stackPtr and not in a register. This is
	 ...
	 *
	 * QUESTION 1: why must setcontext_called be stored on the stackPtr, and not
	...
	/* Get mycontext: make sure to read the man page of getcontext in detail,
	 * or else you will not be able to answer the questions below. */
	err = getcontext(&mycontext);
	assert(!err);

	/* QUESTION 2: which of the fields of mycontext changed due to the call
	...
	if (setcontext_called == 1) {
		/* QUESTION 3: will be get here? why or why not? */
		show_interrupt();
		exit(0);
	}
	...
	/* now, look inside of the mycontext you just saved. */
	...
	/* QUESTIONS 4: how are these parameters passed into the main function?
	...
	/* now, think about the stackPtr */
	/* QUESTIONS 5: Is setcontext_called and err stored on the stackPtr? does the
	 * stackPtr grow up or down? What are the stackPtr related data in
	...
	printf("memory address of the variable err = %p\n",
	       (void *)&err);
	printf("number of bytes pushed to the stackPtr between setcontext_called "
	       "and err = %ld\n", (unsigned long)mycontext.uc_stack.ss_size);

	printf("stackPtr pointer register (RSP) stored in mycontext = 0x%lx\n",
	       (unsigned long)mycontext.uc_mcontext.gregs[REG_RSP]);

	printf("number of bytes between err and the saved stackPtr in mycontext "
	       "= %ld\n", (unsigned long)((unsigned long)&err -
	       (unsigned long)mycontext.uc_mcontext.gregs[REG_RSP]));

	/* QUESTION 6: what is the uc_stack field in mycontext? Note that this
	 * field is used to store an alternate stackPtr for use during signal
	 * handling, and is NOT the stackPtr of the running thread. */
	 ...
	 * change the stackPtr in a mycontext when you manipulate a mycontext to create
	 * a new thread. */
	...
	/* QUESTION 7: why does the program not fail at the assert below? */
	assert(0);
}
...
show_interrupt(void)
{
	...
	/* QUESTION 8: how did we get here if there was an assert above? */
	...
	/* QUESTION 9: what does interrupts_on() do? see interrupt.c */
	interrupts_on();
	...
	/* QUESTION 10: Are interrupts masked (i.e., disabled) in mycontext?
	...
	/* QUESTION 11: which fields of mycontext changed as a result of the
